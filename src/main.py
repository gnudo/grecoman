...
class MainWindow(QMainWindow, Ui_reco_mainwin):
    ...
    def __init__(self):
        ...
        QObject.connect(self.menuLoadOnlyPaganin,
            SIGNAL("triggered()"), lambda param=ParameterWrap.CLA_dict['paganinon'].child_list[:-1]: self.loadSpecificFromConfigFile(param))  # MENU load specific settings -> Paganin
        ...
        field_order = ['afsaccount', 'cons2', 'inputdirectory',
                       'setinputdirectory', 'inputtype', 'prefix',
                       'stitchingtype', 'raws', 'darks', 'flats',
                       'interflats', 'flatfreq', 'preflatsonly', 'roion',
                       'roi_left', 'roi_right', 'roi_upper', 'roi_lower',
                       'binsize', 'scaleimagefactor', 'addpostfix',
                       'cprdirectory', 'setcprdirectory', 'fltpdirectory',
                       'setfltpdirectory', 'sindirectory',
                       'setsinogramdirectory', 'sinograms', 'sinoslider',
                       'recodirectory', 'setrecodirectory', 'jobname',
                       'pag_energy', 'pag_pxsize', 'pag_delta', 'pag_beta',
                       'pag_distance', 'runringremoval', 'waveletfilterdest',
                       'wavelettype', 'waveletpaddingmode',
                       'waveletdecompositionlevel', 'sigmaingaussfilter',
                       'runringremovalstd', 'ring_std_mode', 'ring_std_diff',
                       'ring_std_ringwidth', 'filter', 'cutofffrequency',
                       'edgepadding', 'centerofrotation', 'outputtype',
                       'tifmin', 'tifmax', 'shiftcorrection', 'rotationangle',
                       'geometry', 'zingeron', 'zinger_thresh',
                       'zinger_width', 'cpron', 'withlog', 'paganinon',
                       'fltp_fromtif', 'fltp_fromcpr', 'sinon', 'sin_fromtif',
                       'sin_fromcpr', 'sin_fromfltp', 'steplines',
                       'reconstructon', 'rec_fromtif', 'rec_fromsino',
                       'openinfiji', 'submit', 'jobpriority', 'clearfields',
                       'singleslice', 'print_cmd', 'develbranchon']
        for key in range(len(field_order) - 1):
            self.setTabOrder(getattr(self, field_order[key]),
                             getattr(self, field_order[key + 1]))
