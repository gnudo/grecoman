...
class DatasetFolder(object):
    ...
    def __init__(self, parent):
        ...
        # set computing location based on computer name
        if os.uname()[1] == 'x02da-cons-2':
            parent.cons2.setChecked(1)

        # set location based on PSI/AFS home-dir
        if self.homedir[0:16] == '/afs/psi.ch/user':
    ...
    def merlin2SshfsPath(self, path):
        ...
        splitted_dir = self.splitOsPath(str(path))
        tmp_list = [self.merlin_mount_dir] + \
            splitted_dir[len(split_merlin_base):]
        return os.path.join(*tmp_list)
